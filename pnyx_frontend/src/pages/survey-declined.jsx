...
const Admin = () => {
    ...
    const fetchSurveys = async () => {
        try {
            const response = await axios.get('http://localhost/survey-app/get-declined-surveys.php');
    ...
    const handleApprove = async (survey) => {
        try {
            const response = await axios.post('http://localhost/survey-app/survey-pending.php', {
                id: survey.id,
                action: 'approve'
            });
    ...
    const handleView = async (survey) => {
        try {
            const response = await axios.get(`http://localhost/survey-app/take-survey.php?id=${survey.id}`);
            console.log(response.data);
                    ...
                    {surveys.map((survey) => (
                        <Grid item xs={12} sm={6} md={4} key={survey.id}>
                            ...
                            >
                                ...
                                >
                                    <Button variant="contained" onClick={() => handleView(survey)}>View</Button>
                                </Box>
                                <CardActions
                                    sx={{
                                        marginTop: "auto",
                                ...
                                >
                                    <Button sx={{ color: "green" }} size="small" onClick={() => handleApprove(survey)}>
                                        Approve
                                    </Button>
                                    <Button sx={{ color: "red" }} size="small" onClick={() => handleDecline(survey)}>
                                        Decline
                                    </Button>
                                </CardActions>
                            </Card>
