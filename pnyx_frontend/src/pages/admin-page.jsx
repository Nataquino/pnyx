...
const Admin = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        if (res.data && res.data.users && res.data.surveys) { // Check if the response is structured correctly
  ...
  return (
    <Stack sx={{backgroundColor: "skyblue", height:"100vh"}}>
      <Box>
        <AdminMain />
        <Stack sx={{ backgroundColor: "lightgray", height: "100vh", padding: 4 }}>
          <Box sx={{ marginBottom: 4 }}>
            <Typography variant="h4">Admin Dashboard</Typography>
          </Box>
          {error && ( // Display error message if there is an error
            <Box sx={{ padding: 2, backgroundColor: "red", color: "white", borderRadius: 2, marginBottom: 4 }}>
